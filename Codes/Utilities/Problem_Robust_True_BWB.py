...
# ==========================================================
# Problem - True evaluation of the final ND front using true evaluation (no surrogates)
# ==========================================================

class MyProblem_Robust(Problem):
    ...
    def __init__(
        ...
        self.del_x = del_x
        self.alpha = alpha
        self.nsamples = 90 #  nsamples_scalar*(self.n_design + self.n_random)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if self.del_x == 0 and not self.has_random:
            ...
            F_samples, G = self.concept["func"](x)
        ...
        # ==================================================
        F, G = self.concept["func"](X_flat)
        F_samples = np.asarray(F).reshape(N, ns, self.n_obj)

        # ==================================================
        # Constraint evaluation
        # ==================================================
        if self.n_con > 0:
            G_samples = np.asarray(G).reshape(N, ns, self.n_con)
